...
creates the feedback loop: the signal will be sent through the
...
input/output instructions are for signals. Don't restart the Amplifier
Controller Software on any amplifier during this process. Each one
should continue receiving and sending signals until it halts. All
signals sent or received in this process will be between pairs of
...
settings and feedback loop arrangement. Try every combination of the new
phase settings on the amplifier feedback loop. What is the highest
...
class AmpIntComputer:
    ...
    def op_3(self, code):  # opcode 3: take integer as input
        # Pops first item in self.inputs (phase setting) if length > 1
...
def amp_power(memory, phase_settings):
    ...
    while output is not None:
        ...
        # If phase setting has been popped from self.inputs
        if len(amps[count].inputs) < 2:
