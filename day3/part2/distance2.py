...
class Line:
    def __init__(self, point1, point2):
        ...
        if self.a.x == self.b.x:
            self.midpoints = [Point(self.a.x, y) for y in range(self.a.y+1, self.b.y)]
        else:
            self.midpoints = [Point(x, self.a.y) for x in range(self.a.x+1, self.b.x)]
...
def num_steps(lines, index):
    visited = []
    for i in range(0, index): # try index + 1 if not work
        steps = lines[i].midpoints + [lines[i].b]
        steps = [(p.x, p.y) for p in visited]
        visited.extend(steps)

    return sorted(visited)


def fewest_steps(lines1, lines2, intersections):
    steps = []
...
print(intersections[0], L1[intersections[0][1]])
