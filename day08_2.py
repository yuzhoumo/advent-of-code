...
2 is transparent. The layers are rendered with the first layer in front and the last layer in back. So, if a given
position has a transparent pixel in the first and second layers, a black pixel in the third layer, and a white pixel
in the fourth layer, the final image would have a black pixel at that position. What message is produced after decoding
your image? """
...
class Image:  # Stores x, y dimensions, list of raw digits, and list of layer objects
    ...
    def decode(self):  # Combines all layers into one layer object
        ...
        while layers:  # Overwrites decoded layer with next layer in image
            current = layers.pop()
            for y in range(self.y):
                for x in range(self.x):
                    # Does nothing if pixel is transparent (if value is 2)
                    if current.rows[y][x] == 0 or current.rows[y][x] == 1:  # Overwrite if black or white pixel
...
def main():
    ...
    with open(sys.argv[1], 'r') as f:
        # Read in arguments as x and y dimensions of image
