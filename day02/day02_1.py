...
An Intcode program is a list of integers separated by commas. To run one, start by looking at the first integer (called
position 0). Here, you will find an opcode - either 1, 2, or 99. The opcode indicates what to do. Encountering an
...
def main():
    ...
    memory[1], memory[2] = int(sys.argv[2]), int(sys.argv[3])  # Sets memory values at index 1 & 2 with passed in args

    print(IntComputer(memory).run()[0])  # Prints memory value at index 0 after execution
