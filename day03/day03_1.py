...
Two wires are connected to a central port and extend outward on a grid. You trace the path each wire takes as it leaves
...
count, nor does a wire count as crossing with itself. What is the Manhattan distance from the central port to the
...
class Point:
    ...
    def taxi_dist(self, other):  # Returns manhattan distance (only straight lines parallel to axes) between 2 points
...
class Line:
    def __init__(self, a, b):
        assert isinstance(a, Point), 'Endpoint a is not a valid point object'
        assert isinstance(a, Point), 'Endpoint b is not a valid point object'
        # Sets endpoints of line
        self.a, self.b = a, b

    def get_intersect(self, other):
        # Determines vertical or horizontal lines assuming line segments are parallel to either x or y axes
        if self.a.x == self.b.x and other.a.y == other.b.y:
            ...
            return None  # Lines are parallel to each other or one or both are not parallel to either x or y axes

        # Returns intersection point if it exists, otherwise None
        x, y = vertical[0], horizontal[0]
        if min(horizontal[1]) < x < max(horizontal[1]) and min(vertical[1]) < y < max(vertical[1]):
            return Point(x, y)
...
def gen_points(instructions):
    ...
    for instruction in instructions:
        prev_x, prev_y = points[-1].x, points[-1].y  # Gets coordinates of previous point
        direction, dist = instruction[0], int(instruction[1:])  # Gets direction and distance value

        # Sets next point based on direction and distance from prev point
        if direction == 'U':  # Up
...
def gen_lines(points):  # Generates a list of line objects based on a list of endpoints
    lines = []
...
def intersects(lines1, lines2):  # Assumes both lines consist of segments parallel to either x or y axes
    results = []
    for first in lines1:
...
def main():
    ...
    instructions1, instructions2 = instructions[0].split(','), instructions[1].split(',')

    line1, line2 = gen_lines(gen_points(instructions1)), gen_lines(gen_points(instructions2))
