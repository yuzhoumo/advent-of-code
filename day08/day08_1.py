...
pixel of the image is filled. Each image actually consists of a series
of identically-sized layers that are filled in this way. So, the first
digit corresponds to the top-left pixel of the first layer, the second
digit corresponds to the pixel to the right of that on the same layer,
and so on until the last digit, which corresponds to the bottom-right
pixel of the last layer.

The image you received is 25 pixels wide and 6 pixels tall. To make sure
the image wasn't corrupted during transmission, the Elves would like you
to find the layer that contains the fewest 0 digits. On that layer, what
is the number of 1 digits multiplied by the number of 2 digits? """


class Image:
    # Stores x, y dimensions, list of raw digits, and list of layer
    # objects
    ...
    def add_layer(self, layer):
        assert layer.x == self.x and layer.y == self.y, ('Layer size '
                                           'does not match image size')
        self.layers.append(layer)

    def set_data(self, data):
        assert len(data) % (self.x * self.y) == 0, ('Data length does '
                                                   'not match img size')
...
class Layer:
    # Stores x, y dimensions, list of raw digits, and 2d list of layer
    # data
    def __init__(self, x, y):
        self.x, self.y = x, y
    ...
    def add_row(self, row):
        assert len(row) == self.x, ('Row length does not match x '
                                    'dimension')
        self.rows.append(row)

    def set_data(self, data):
        assert len(data) == self.x * self.y, ('Data length does not '
                                              'match layer size')
        self.raw_data = [int(n) for n in data]
        data = list(self.raw_data)
    ...
    # Counts how many times digit d appears in self.raw_data
...
def corruption_check(image):
    ...
    fewest_zeros_layer = \
        min(image.layers, key=lambda x: Layer.count_digit(x, 0))
    # Returns number of 1 digits * number of 2 digits on layer with
    # fewest zeros
    return fewest_zeros_layer.count_digit(1) * \
        fewest_zeros_layer.count_digit(2)
