...
To complete the gravity assist, you need to determine what pair of inputs produces the output 19690720. The inputs
should still be provided to the program by replacing the values at addresses 1 and 2, just like before. In this program,
the value placed in address 1 is called the noun, and the value placed in address 2 is called the verb. Each of the two
    ...
    def run(self):
        while self.ptr < len(self.memory):
            if self.memory[self.ptr] == 1:
                self.op_1(self.memory[self.ptr + 1], self.memory[self.ptr + 2], self.memory[self.ptr + 3])
                self.ptr += 4
            elif self.memory[self.ptr] == 2:
                self.op_2(self.memory[self.ptr + 1], self.memory[self.ptr + 2], self.memory[self.ptr + 3])
                self.ptr += 4
...
def main():
    ...
    assert len(sys.argv) > 2, 'Missing argument: expected output integer of IntComputer'
