...
matching digits. Given this additional criterion, but still ignoring the range rule, the following are now true:
...
def is_valid_password(n, adj_counts):
    ...
    while n:  # Parses number right to left
        ...
        if d > prev:
            return False
        if d == prev:
            adjacent[d] += 1  # Keeps track how many same adjacent values for each digit
        prev, n = d, n // 10

    return 1 in adjacent  # Is valid if passes above and any digit has exactly 1 identical adjacent value


def count(lower, upper):
    cnt, adj_counts = 0, tuple(0 for i in range(0, 10))
    for n in range(lower, upper):
        if is_valid_password(n, list(adj_counts)):  # adj_counts tracks number of same adjacent values for each digit
            cnt += 1
    return cnt
