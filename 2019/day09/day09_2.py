...
You now have a complete Intcode computer. You just need to boost your
sensors using the BOOST program. The program runs in sensor boost mode
by providing the input instruction the value 2. Once run, it will boost
the sensors automatically, but it might take a few seconds to complete
the operation on slower hardware. In sensor boost mode, the program will
output a single value: the coordinates of the distress signal. Run the
BOOST program in sensor boost mode. What are the coordinates of the
distress signal? """


class IntComputer:
    def __init__(self, user_inputs, memory):
        # user_inputs is a list of ints popped from each time
        # input is needed
        
        # Convert list to dict with indices as keys
        self.memory = dict(zip(range(len(memory)), memory))
    ...
    def run(self):
        while self.get_memory(self.ptr) != 99:
            buffer = \
                [0 for _ in range(5 - len(str(self.get_memory(self.ptr))))]
    ...
    def get_memory(self, index):
        # Gets memory at specified index even if index > len(self.memory)
    ...
    def set_memory(self, index, value):
        # Sets memory at specified index even if index > len(self.memory)
    ...
    def index_val(self, mode, i):
        # Returns index value based on parameter mode
        if mode == 0:
            return self.get_memory(i)
        if mode == 2:
    ...
    def op_1(self, code):  # opcode 1: addition
        self.set_memory(self.index_val(code[0], self.ptr + 3),
                        self.val(code[2], self.ptr + 1) + self.val(code[1],
                        self.ptr + 2))
        self.ptr += 4
    ...
    def op_8(self, code):
        # opcode 8: sets memory at param c to 1 if param a == b,
        # else sets memory to 0
        if self.val(code[2], self.ptr + 1) == self.val(code[1], self.ptr + 2):
            self.set_memory(self.index_val(code[0], self.ptr + 3), 1)
        else:
            self.set_memory(self.index_val(code[0], self.ptr + 3), 0)
